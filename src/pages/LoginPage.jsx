...
import {
  ...
  signInWithPopup,
  signInWithRedirect,
  signInAnonymously, // Importar el método de inicio de sesión anónimo
} from "firebase/auth";
...
const LoginPage = () => {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      console.error("LoginPage: Error al iniciar el proceso de login:", err);
      if (err.code === "auth/popup-closed-by-user") {
        setError("El proceso de inicio de sesión fue cancelado.");
      } else {
        setError("No se pudo iniciar el proceso de sesión.");
      }
      setIsLoading(false);
    }
  };

  // Nueva función para el inicio de sesión como invitado
  const handleGuestLogin = async () => {
    ...
    try {
      ...
      console.error(
        "LoginPage: Error en el inicio de sesión de invitado:",
        err
      );
